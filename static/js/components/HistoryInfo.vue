<template>
    <div class="info-bar flex-box">
        <div class="history-info-item flex-box">
            <svg width="14" height="14" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                <path d="M1024 544v448q0 14-9 23t-23 9H672q-14 0-23-9t-9-23v-64q0-14 9-23t23-9h224V544q0-14 9-23t23-9h64q14 0 23 9t9 23zm416 352q0-148-73-273t-198-198-273-73-273 73-198 198-73 273 73 273 198 198 273 73 273-73 198-198 73-273zm224 0q0 209-103 385.5T1281.5 1561 896 1664t-385.5-103T231 1281.5 128 896t103-385.5T510.5 231 896 128t385.5 103T1561 510.5 1664 896z"/>
            </svg>
            <span class="history-time">{{FormatDate(created)}}</span>
        </div>
        <div class="history-info-item flex-box">
            <svg width="14" height="14" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                <path d="M512 896v512h-256v-512h256zm384-512v1024h-256v-1024h256zm1024 1152v128h-2048v-1536h128v1408h1920zm-640-896v768h-256v-768h256zm384-384v1152h-256v-1152h256z"/>
            </svg>
            <span class="history-info-clicks">{{clicks}}</span>
        </div>
        <div class="history-info-item flex-box">
            <svg width="14" height="14" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                <path d="M384 448q0-26-19-45t-45-19-45 19-19 45 19 45 45 19 45-19 19-45zm1152 576q0-35-21.5-81t-53.5-47q15-17 25-47.5t10-55.5q0-69-53-119 18-32 18-69t-17.5-73.5T1396 479q5-30 5-56 0-85-49-126t-136-41h-128q-131 0-342 73-5 2-29 10.5T681.5 352t-35 11.5-38 11-33 6.5-31.5 3h-32v640h32q16 0 35.5 9t40 27 38.5 35.5 40 44 34.5 42.5 31.5 41 23 30q55 68 77 91 41 43 59.5 109.5T954 1579t38 85q96 0 128-47t32-145q0-59-48-160.5t-48-159.5h352q50 0 89-38.5t39-89.5zm128 1q0 103-76 179t-180 76h-176q48 99 48 192 0 118-35 186-35 69-102 101.5T992 1792q-51 0-90-37-34-33-54-82t-25.5-90.5T805 1498t-31-64q-48-50-107-127-101-131-137-155H256q-53 0-90.5-37.5T128 1024V384q0-53 37.5-90.5T256 256h288q22 0 138-40 128-44 223-66t200-22h112q140 0 226.5 79t85.5 216v5q60 77 60 178 0 22-3 43 38 67 38 144 0 36-9 69 49 74 49 163z"/>
            </svg>
            <span class="history-info-flags">{{flags}}</span>
        </div>
    </div>
</template>

<style lang="sass">
    @import '../../css/header.scss';
    
    .info-bar{
        height: 25px;
        justify-content: flex-end;
        
        & > .history-info-item{
            align-items: center;
            & > svg{
                margin-right: 5px;
                fill: #b3b3b3;
            }
            & > span{
                font-family: $pt-sans;
                color: $dark-text;
            }
            &:not(:last-child){
                margin-right: 20px;
            }
        }
    }
</style>

<script>
    import {FormatDate} from '../filters/';
    export default{
        name: "history-info",
        
        props: ['created', 'clicks', 'flags'],
        
        methods:{
            FormatDate: FormatDate
        }
    }
</script>